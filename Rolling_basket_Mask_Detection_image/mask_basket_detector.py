...
def mask_detection(inference_path, frozen_graph_path, labelmap, number_of_classes, input):
    ...
    try:
        ...
        print("There is no images find in this directory", e)
...
def class_selection(scores, classes, image, category_index, boxes):
    ...
    new_classes = classes.flatten().astype(np.int32)
    ...
    try:
        if 3 in new_class and 1 in new_class:
            ...
            cv2.imwrite('basket_detection_1.jpg', k)
            cv2.waitKey(0)
            cv2.destroyAllWindows()

        elif 1 not in new_class or 3 not in new_class:
            k = vis_util.visualize_boxes_and_labels_on_image_array(image,
                                                                   np.squeeze(boxes),
                                                                   np.squeeze(classes).astype(np.int32),
                                                                   np.squeeze(scores),
